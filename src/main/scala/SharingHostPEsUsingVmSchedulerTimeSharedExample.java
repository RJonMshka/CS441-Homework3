...
public class SharingHostPEsUsingVmSchedulerTimeSharedExample {
    ...
    class CustomVmScheduling extends VmSchedulerAbstract {
        public CustomVmScheduling() {
            super(0.1d);
        }

        public CustomVmScheduling(double factor) {
            super(factor);
        }
        ...
        @Override
        protected boolean allocatePesForVmInternal(Vm vm, MipsShare mipsShareRequested) {
            if (!isSuitableForVm(vm, mipsShareRequested)) {
                return false;
            }
        ...
        protected void allocateMipsShareForVm(Vm vm, MipsShare mipsShareRequested) {
            MipsShare mipsShare = new MipsShare(mipsShareRequested.pes(), mipsShareRequested.mips());
...
//            this.cloudletList.add(cloudlet);
        ...
        for(int i = 0; i < NUMBER_OF_VMS + 2; i++){
    ...
    private Host createHost() {
        ...
        return new HostSimple(ram, bw, storage, peList)
                ...
                .setVmScheduler(new CustomVmScheduling());
    }
    ...
    private Cloudlet createCloudlet(DatacenterBroker broker) {
        ...
        return new CloudletSimple(numberOfCreatedCloudlets++, CLOUDLET_LENGTH, 1)
                .setFileSize(fileSize)
                .setOutputSize(outputSize)
                .setUtilizationModel(utilization);
    }
